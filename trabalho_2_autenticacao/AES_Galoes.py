...
# Texto de entrada de 16 bytes (bloco)
opcao = int(input("Digitar texto ou não?\n(Digite 1 para a primeira opção e 2 para a segunda)\n"))
if opcao == 1:
    plaintext = bytes(input(), 'utf-8')
else:
    plaintext = b'Professor gondin 21'

# Substituição da chave
key_schedule = expancion_key(key)

# Texto cifrado (inicialmente igual ao texto de entrada)
ciphertext = bytearray(plaintext)

# Número de rounds
num_rounds = 10

# Adiciona chave inicial
ciphertext = xor_bytes(ciphertext, key_schedule[:16])
